...
export default class MysqlDriver extends AbstractDriver {
    ...
    entities.forEach((ent) => {
      response
        ...
        .forEach((resp) => {
          ...
          if (resp.COLUMN_TYPE.endsWith(" unsigned")) options.unsigned = true;
  ...
  public async GetIndexesFromEntity(
    ...
    entities.forEach((ent) => {
      ...
      indexNames.forEach((indexName) => {
        ...
        if (records[0].is_fulltext === 1) indexInfo.options.fulltext = true;
  ...
  private static ReturnDefaultValueFunction(
    ...
    if (defaultValue === "CURRENT_TIMESTAMP" || defaultValue.startsWith(`'`)) {
      return `() => "${defaultValue}"`;
    }
    if (dataType === "set") {
